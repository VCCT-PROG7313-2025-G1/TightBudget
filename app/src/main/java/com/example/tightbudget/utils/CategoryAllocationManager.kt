...
class CategoryAllocationManager(private val database: AppDatabase) {
     ...
     */
    suspend fun loadCategoryAllocations(budgetGoalId: Int): List<CategoryBudgetItem> = withContext(Dispatchers.IO) {
        try {
            ...
            return@withContext categoryBudgets.map { categoryBudget ->
                ...
                CategoryBudgetItem(
                    ...
                    emoji = category?.emoji ?: EmojiUtils.getCategoryEmoji(categoryBudget.categoryName),
     ...
     */
    suspend fun createDefaultAllocations(totalBudget: Double): List<CategoryBudgetItem> = withContext(Dispatchers.IO) {
        try {
            val categoryDao = database.categoryDao()
            val allCategories = categoryDao.getAllCategories()
    ...
    suspend fun getCategorySpendingStats(
        ...
        try {
            ...
            calendar.set(year, month - 1, calendar.getActualMaximum(Calendar.DAY_OF_MONTH), 23, 59, 59) // Last day of month
